...
def getUserSpamScore(user):
    ...
    return {"score": score, "color": color, "emoji": icon}
...
def getUserSpamPercent(user):
    ...
    if found:
        ...
        return {"score": r.text, "color": "#FF0000", "emoji": "Terrible"}
...
def genRandomStatus(carp=False):
    if not carp:
        statuses = ['primary', 'secondary', 'success', 'danger', 'warning']
    else:
        statuses = ['primary', 'success', 'danger', 'warning', 'info']
...
def saveChartData(**z):
    ...
    if r.text:
        with open(f'rc/app/static/me/charts/json/{cName}-{uTotal}.json',
                  'w') as fh:
            fh.write(r.text)
        return True
    else:
